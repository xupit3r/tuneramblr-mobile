...
import tjs.tuneramblr.meta.location.utils.LegacyLastLocationFinder;
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (intent.hasExtra(key)) {
			...
			long lastTime = prefs.getLong(
					TuneramblrConstants.SP_KEY_LAST_LIST_UPDATE_TIME,
					Long.MIN_VALUE);
			long lastLat = prefs.getLong(
					TuneramblrConstants.SP_KEY_LAST_LIST_UPDATE_LAT,
					Long.MIN_VALUE);
			long lastLng = prefs.getLong(
					TuneramblrConstants.SP_KEY_LAST_LIST_UPDATE_LNG,
					Long.MIN_VALUE);
			Location lastLocation = new Location(
					TuneramblrConstants.CONSTRUCTED_LOCATION_PROVIDER);
			...
			if ((lastTime > System.currentTimeMillis()
					...
					|| (lastLocation.distanceTo(location) < TuneramblrConstants.MAX_DISTANCE))
				location = null;
		}
		...
		if (location != null) {
			Log.d(TAG, "Passivly updating place location.");
			// Intent updateServiceIntent = new Intent(
			// context,
			// TuneramblrConstants.SUPPORTS_ECLAIR ?
			// EclairPlacesUpdateService.class
			// : PlacesUpdateService.class);
			// updateServiceIntent.putExtra(
			// TuneramblrConstants.EXTRA_KEY_LOCATION, location);
			// updateServiceIntent.putExtra(TuneramblrConstants.EXTRA_KEY_RADIUS,
			// TuneramblrConstants.DEFAULT_RADIUS);
			// updateServiceIntent.putExtra(
			// TuneramblrConstants.EXTRA_KEY_FORCEREFRESH, false);
